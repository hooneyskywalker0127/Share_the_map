...
위는 맵의 물건 배치를 섞는 `randomize_map.py` 를 실행하기 전(왼쪽)과 후(오른쪽)를
같은 카메라로 비교한 장면입니다. Isaac Sim에서 맵을 연 상태로 Window > Script Editor
에 아래 두 줄을 넣고 Ctrl+Enter 하면 열려 있는 스테이지가 그 자리에서 바뀝니다.

```python
p = "randomize_map.py"          # 이 저장소를 받은 경로로 바꿔 주십시오
exec(open(p).read())
```

한 번 실행하면 낱개 물건마다 이런 변화가 일어납니다.

- 자리를 옮깁니다. 랙에 실린 화물은 원래 얹혀 있던 단 안에서, 바닥 물건은 원래 자리 반경 안에서 움직입니다.
- 방향을 돌립니다. 바닥 물건은 자유롭게, 랙 화물은 조금만 틀어집니다.
- 가끔 바닥이 아니라 다른 물건 위에 얹힙니다.
- 가끔 장면에서 아예 빠집니다. 다시 실행하면 되살아난 뒤 다시 섞이므로, 돌릴수록 맵이 비지는 않습니다.

벽·기둥·랙 프레임·선반·바닥 도색·통로 같은 구조물은 움직이지 않고, 물건끼리 겹치게
놓지도 않습니다. 콘솔에 어떤 물건이 어디에서 어디로 갔는지 전후 좌표가 찍힙니다.
자세한 내용은 아래 물건 배치 섞기 항목에 있습니다.
