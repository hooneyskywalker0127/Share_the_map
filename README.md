...
[![전체 영상 재생](docs/youtube_thumb.jpg)](https://youtu.be/JSn95m4CYlo)

위 이미지를 누르면 YouTube에서 재생됩니다 — https://youtu.be/JSn95m4CYlo
...
터미널에 `[open_map] prims on stage: 690` 이 찍히면 정상입니다.
...
| `/World/TwinHall/Hall_B` | Hall_A를 X로 +26.30 m 옮긴 복사본 |
| `/World/TwinHall/Junction` | 공유 벽(통로 위아래 두 구간) + 문설주 + 바닥 표시 |
| `/World/TwinHall/PhysicsScene` | 중력 −Z 9.81 |
| `/World/TwinHall/Dome` | 앰비언트 돔 라이트 |

홀 안에는 주 통로와 작업 구역 도색, 뒤쪽 좁은 지름길(낮은 턱으로 막힘), 북동쪽
창고 구역(랙 + 크레이트 더미), 남쪽 피킹 베이(선반 위와 바닥에 흩어놓은 상자·캔·
파우치), 팩존과 핸드오버 표시, 주 통로를 막고 선 고장 유닛, 기둥과 조명이 있습니다.
조립 스테이션과 컨베이어는 없습니다.
